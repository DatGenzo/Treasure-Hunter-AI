"""
alpha_beta.py — Alpha-Beta minimax adversarial search algorithm implementation.
"""
...
class AlphaBetaAlgorithm(AIAlgorithm):
    """Alpha-Beta minimax turn-based search solver.

    Performs tree search with alpha-beta pruning to speed up decisions.
    """
    ...
    def step(self) -> StepResult:
        """Execute pathfinding or alpha-beta search on the combat state."""
        ...
        for action in get_actions():
            ...
            # Depth 1: Monster turn (Minimizing)
            val = self._alphabeta_tracked(successor, depth=1, alpha=alpha, beta=beta, maximizing=False, tree_nodes=tree_nodes, pruned_nodes=pruned_nodes, path=[action])
            tree_nodes.append({
                "depth": 1,
                "action": action,
                "value": val,
    ...
    def _alphabeta_tracked(self, state: CombatState, depth: int, alpha: float, beta: float, maximizing: bool, tree_nodes: list, pruned_nodes: list, path: list) -> float:
        """Recursive alpha-beta pruned search evaluation tracking nodes."""
        ...
        if maximizing:
            ...
            for idx, action in enumerate(actions):
                ...
                self._nodes_evaluated += 1
                val = self._alphabeta_tracked(successor, depth - 1, alpha, beta, False, tree_nodes, pruned_nodes, path + [action])
                ...
                if beta <= alpha:
                    ...
                    # Track remaining actions as pruned
            ...
            for idx, action in enumerate(actions):
                successor = get_successor(state, action)
                self._nodes_evaluated += 1
                val = self._alphabeta_tracked(successor, depth - 1, alpha, beta, True, tree_nodes, pruned_nodes, path + [action])
    ...
    def _alphabeta(self, state: CombatState, depth: int, alpha: float, beta: float, maximizing: bool) -> float:
        """Recursive alpha-beta pruned search evaluation."""
        if depth == 0 or state.player_hp <= 0 or state.monster_hp <= 0:
            return evaluate_state(state)
        ...
        if maximizing:
            ...
            for action in get_actions():
                successor = get_successor(state, action)
                self._nodes_evaluated += 1
                val = self._alphabeta(successor, depth - 1, alpha, beta, False)
                max_val = max(max_val, val)
                alpha = max(alpha, max_val)
                if beta <= alpha:
                    self._pruned_count += 1
            ...
            for action in get_actions():
                successor = get_successor(state, action)
                self._nodes_evaluated += 1
                val = self._alphabeta(successor, depth - 1, alpha, beta, True)
                min_val = min(min_val, val)
                beta = min(beta, min_val)
                if beta <= alpha:
                    self._pruned_count += 1

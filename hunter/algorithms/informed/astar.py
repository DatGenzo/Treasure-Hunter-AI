...
class AStarAlgorithm(AIAlgorithm):
    ...
    Combines path cost g(n) and heuristic estimate h(n) to find optimal path efficiently.
    """
    ...
    def initialise(
        ...
        h_start = self._heuristic(start[0], start[1])
        self._pq.push(h_start, start)
        self._g_score = {start: 0.0}
        self._f_score = {start: h_start}
    ...
    def get_vis_data(self) -> Dict[str, Any]:
        """Format visualisation dictionary, exposing A* specific open/closed sets."""
        open_set = {node for _, _, node in self._pq._heap}
        return {
            "open_set": open_set,
            "closed_set": self._closed_set,
            "current": self._current_node,
            "path": self._path,
            "f_values": self._f_score.copy(),
        }
    ...
    def step(self) -> StepResult:
        ...
        if self._phase == "SEARCHING":
            while True:
                ...
                curr = self._pq.pop()
                if curr not in self._closed_set:
                    break

            self._closed_set.add(curr)
            ...
            for neighbor_vec in self._tilemap.neighbors(curr[0], curr[1], treat_goal_as_walkable=self._goal):
                ...
                g_tentative = self._g_score[curr] + self._tilemap.move_cost(n[0], n[1])

                if n not in self._g_score or g_tentative < self._g_score[n]:
                    self._g_score[n] = g_tentative
                    f_val = g_tentative + self._heuristic(n[0], n[1])
                    self._f_score[n] = f_val
                    self._parent[n] = curr
                    self._pq.push(f_val, n)
                    if n not in self._closed_set:
                        self._stats_obj.nodes_visited += 1

            # Update memory peak
            open_set_size = len({node for _, _, node in self._pq._heap})
            self._stats_obj.memory_peak = max(self._stats_obj.memory_peak, len(self._closed_set) + open_set_size)

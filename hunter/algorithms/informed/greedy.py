"""
greedy.py — Greedy Best-First Search algorithm implementation.
"""
...
class GreedyAlgorithm(AIAlgorithm):
    """Greedy Best-First Search (GBFS) pathfinder.

    Expands nodes based solely on the heuristic estimate h(n) to the goal. Fast but not optimal.
    """
    ...
    def initialise(
        ...
        """Set up Greedy Search variables."""
        self._start = start
        self._goal = goal
        ...
        self._pq.push(self._heuristic(start[0], start[1]), start)
        self._visited = set()
    ...
    def step(self) -> StepResult:
        ...
        if self._phase == "SEARCHING":
            ...
            for neighbor_vec in self._tilemap.neighbors(curr[0], curr[1], treat_goal_as_walkable=self._goal):
                ...
                if n not in self._visited and n not in self._parent:
                    self._parent[n] = curr
                    priority = self._heuristic(n[0], n[1])
                    self._pq.push(priority, n)
